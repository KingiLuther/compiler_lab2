...
// 用于存储类型信息的映射。如果需要，可以设计新的数据结构。
typeMap g_token2Type;         // 全局标记ID到类型的映射
typeMap funcparam_token2Type; // 函数参数标记ID到类型的映射，临时变量
typeMap func2retType;         // 函数名到返回类型的映射

paramMemberMap func2Param;     // 函数名到函数参数声明的映射，全局存储
paramMemberMap struct2Members; // 结构体名到结构体参数声明的映射

std::unordered_map<std::string, std::string> funcDecl; // 函数声明

int cur = 1; // 当前处理的标记ID

std::string cur_func_name = ""; // 当前处理的函数名
